...
func _handle_arrival_immediate() -> void:
...
func _handle_arrival() -> void:
	...
	_has_target = false
	if _has_look_target:
		var face_dir := _look_target - global_position
		face_dir.y = 0.0
		if face_dir.length() > 0.01:
			visual_node.rotation.y = atan2(face_dir.x, face_dir.z)
